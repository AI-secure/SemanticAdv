...
def main(config):
    ...
    for i_target in target_list:
        ...
        index = i_target - 2000
        save_image(
            'paper_results_layer02/' + str(index) + '_' + str(i_target) + '_' +
        ...
        for z in range(300):
            denormed_adv = solver.h02(delta, x_real, c_org)
            edit_final = solver.f02(denormed_adv)
        ...
        with torch.no_grad():
            generated_ori = solver.h02(delta, new_x_real, c_org)
        ...
        for j in range(17):
            ...
            with torch.no_grad():
                denormed_adv = solver.h02(delta, new_x_real, c_org)
    ...
    if not osp.exists('./paper_results_layer02/'):
        os.makedirs('./paper_results_layer02/')
    f = open('paper_results_layer02/record.txt', 'w')
