...
def main(config):
    ...
    for i_target in target_list:
        ...
        index = i_target - 2000
        save_image(
            'paper_results_layer1/' + str(index) + '_' + str(i_target) + '_' +
        ...
        for z in range(300):
            denormed_adv = solver.h1(delta, x_real, c_org)
            edit_final = solver.f1(denormed_adv)
        ...
        with torch.no_grad():
            generated_ori = solver.h1(delta, new_x_real, c_org)
        ...
        for j in range(17):
            ...
            with torch.no_grad():
                denormed_adv = solver.h1(delta, new_x_real, c_org)
    ...
    if not osp.exists('./paper_results_layer1/'):
        os.makedirs('./paper_results_layer1/')
    f = open('paper_results_layer1/record.txt', 'w')
