...
class GlobalGenerator(nn.Module):
    def __init__(self,
        ...
        model = [
            ...
            self.norm_layer(ngf), activation
        ]
        ### downsample
...
class GlobalTwoStreamGenerator(nn.Module):
    def __init__(self,
                 ...
                 which_stream='ctx',
                 use_output_gate=False,
                 feat_fusion='early_add',
                 extra_embed=False):
        assert (n_blocks >= 0)
        assert (not (not ('label' in which_stream) and
                     ('late' in feat_fusion)))
        ...
        if which_stream == 'ctx_label':
            self.mask_downsampler = nn.MaxPool2d(kernel_size=2**n_downsampling,
                                                 stride=2**n_downsampling)
            self.feat_fuser = FeatureFusionBlock(feat_dim, feat_dim,
                                                 feat_fusion.split('_')[1],
                                                 self.norm_layer, activation,
                                                 Identity())
        if 'early' in feat_fusion:
            ...
            self.obj_latent_embedder = self.get_embedder(
                feat_dim, n_blocks_feat_embed)
            self.ctx_latent_embedder = self.get_embedder(
                feat_dim, n_blocks_feat_embed)
            self.latent_embedder = self.get_embedder(feat_dim,
                                                     n_blocks_feat_combine)
    ...
    def get_input(self, input_nc):
        model = [
            ...
            self.norm_layer(self.ngf), self.activation
        ]
    ...
    def get_downsampler(self):
        ...
        for i in range(self.n_downsampling):
            mult = 2**i
            model += [
                nn.Conv2d(self.ngf * mult,
                          self.ngf * mult * 2,
                          kernel_size=3,
                          stride=2,
                          padding=1),
                self.norm_layer(self.ngf * mult * 2), self.activation
            ]
        return nn.Sequential(*model)
        ...
        for i in range(self.n_downsampling):
            ...
            model += [
                nn.ConvTranspose2d(dim_in,
                                   ...
                                   padding=1,
                                   output_padding=1),
                self.norm_layer(int(self.ngf * mult / 2)), self.activation
            ]
        return nn.Sequential(*model)
    ...
    def forward_encoder(self, inputEmbedder, encoder, input, use_skip):
        ...
        for i, layer in enumerate(encoder):
            ...
            if use_skip and ((i < self.n_downsampling * 3 - 1) and
                             (i % 3 == 2)):  # super-duper hard-coded
    ...
    def forward_decoder(self, decoder, outputEmbedder, dec_feat, enc_feats):
        for i, layer in enumerate(decoder):
            if (self.use_skip and len(enc_feats) > 0) and (
                (i > 0) and (i % 3 == 0)):  # super-duper hard-coded
                dec_feat = torch.cat(
                    (enc_feats[-int((i - 3) / 3) - 1], dec_feat), 1)
    ...
    def forward(self, img, label, mask):
        ...
        output = self.forward_decoder(self.decoder, self.outputEmbedder,
                                      embed_feat, ctx_feats)
        if self.use_output_gate:
            mask_output = mask.repeat(1, self.output_nc, 1, 1)
            #output = (1-mask_output)*img + mask_output*output
            output = (1 -
                      mask_output) * img[:, :3, :, :] + mask_output * output
    ...
    def g_double_out(self, embed_feat, embed_feat2, ctx_feats, img, mask,
                     mask2):
        output = self.forward_decoder(self.decoder, self.outputEmbedder,
                                      embed_feat, ctx_feats)
        output2 = self.forward_decoder(self.decoder, self.outputEmbedder,
                                       embed_feat2, ctx_feats)
        if self.use_output_gate:
            mask_output = mask.repeat(1, self.output_nc, 1, 1)
            ...
            output = (
                1 - mask_output - mask_output2
            ) * img[:, :3, :, :] + mask_output * output + mask_output2 * output2
    ...
    def forward(self, input, inst):
        ...
        for i in inst_list:
            ...
            for j in range(self.output_nc):
                ...
                outputs_mean[indices[:, 0], indices[:, 1] + j, indices[:, 2],
                             indices[:, 3]] = mean_feat
        return outputs_mean
